...
public class RegistrationIntentService extends IntentService {
    ...
    private class GcmPushToken extends AsyncTask <String,String,String> {
        ...
        protected String doInBackground(String... params) {
            ...
            profile = UserProfile.readProfileFromPrefs(RegistrationIntentService.this);

            if (profile.isProfileCreated()) {
                HashMap<String, String> pairs = new HashMap<>();
                pairs.put("client", profile.getLogin());
                pairs.put("password", profile.getPassword());
                pairs.put("os", Constants.APP_TYPE);
                pairs.put("token", token);
                pairs.put("hash", EncryptUtils.sha256(
                        getResources().getString(R.string.auth_register) +
                                profile.getLogin() +
                                profile.getPassword() +
                                Constants.APP_TYPE +
                                token));
